...
for iii=1:length(regresssets)
    ...
    noiseMasks(1,[5]) = 0;% white noise only
    noiseMasks(2,[1 5]) = 0;% white noise only
    noiseMasks(3,[3 5]) = 0;% white noise only
 ...
 % find average height for each 1000 years -10050 = 12kaBP
    ...
    [f2s,sd2s,V2s,testlocs,logp(ii,jj),passderivs,invcv]=RegressHoloceneDataSets(datasets{1},testsitedef,modelspec(trainspecs(jj)),thetTGG{jj},trainsub,noiseMasks(:,:),testt,refyear,collinear);
...
xl=[-10000 2000];
    wtestlocs=testlocs;
    subsites=[1];
    wdataset.offset=0*Y; Y0s=wdataset.Y0; testreg=testlocs.reg;
