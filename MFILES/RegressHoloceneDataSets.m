...
%    logp - log likelihood
%
% Last updated by Erica Ashe, Wednesday Aug 1 2018
...
if size(thetTGG,1) > size(thetTGG,2) 
    thetTGG=thetTGG';
    %% warning message?
end
...
if length(meantime)<length(trainsub)
    trainsub=1:length(meantime);
end    
...
compactcorr=0.*dataset.compactcorr;
time1=dataset.time1;
time2=dataset.time2;
dY=dataset.dY;
...
if length(trainsub)<length(meantime)
    ...
    dt1t1=dYears(meantime,meantime);
    dy1y1 = dDist([lat long],[lat long]);
    ...
    if max(dt)>0
        if analyticalderiv
            ...
            if length(meantime)>length(trainsub)
                ...
                [dK,df,d2f,yoffset] = GPRdx(meantime,Y,dt,dY,mspec,1,[1:length(meantime)]');
            end    
        else
            ...
            if length(meantime)>length(trainsub)
                ...
                %mspec,1,[1:length(trainsub)]');
            else
                [dK,df,d2f,yoffset] = GPRdx(meantime,Y,dt,dY,@(x1,x2,r1,r2) wcvfunc(x1,x2,thetTGG,r1,r2),1,[1:length(trainsub)]');
                %mspec,1,[1:length(meantime)]');
            end    
        end
